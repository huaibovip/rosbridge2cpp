...
namespace rosbridge2cpp{
  bool SocketTCPConnection::Init(std::string p_ip_addr, int p_port){
	  ...
	  if (win_error != 0) {
		  std::cerr << "win_error = " << win_error << std::endl;
	  }
	  
	  if (HIBYTE(wsaData.wVersion) != 2 || LOBYTE(wsaData.wVersion) != 2)
	  {
		  WSACleanup();
		  std::cerr << "[TCPConnection] Version request error\n";
		  return false;
	  }
  ...
  bool SocketTCPConnection::SendMessage(const uint8_t *data, unsigned int length){
    if( send(sock_ , (char*)data , length , 0) < 0)
    {
      perror("[TCPConnection] Send failed : ");
      return false;
    }
  ...
  int SocketTCPConnection::ReceiverThreadFunction(){
    ...
    while(!terminate_receiver_thread_){
      std::cout << "." << std::endl;
      int count = recv(sock_, (char*)recv_buffer.get(), buf_size, 0);
      ...
      if(count < 0){
        if(!terminate_receiver_thread_)
          ReportError(TransportError ::SOCKET_ERRORS);

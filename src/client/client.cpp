...
void connection_error_handler(TransportError err) {
  ...
  if(err == TransportError ::SOCKET_ERRORS)
...
//  ros.enable_bson_mode();
  ...
  std::string IP;
  std::cout << "Please input your machine IP :" << std::endl;
  std::cin >> IP;
  ...
  ROSTopic sub_topic(ros, "/sub_test", "std_msgs/String");
  ...
  sub_topic.Subscribe([&](ROSBridgePublishMsg& msg) -> void { /*std::cerr << Helper::get_string_from_rapidjson(msg.ToJSON(d.GetAllocator())) << std::endl; */});
